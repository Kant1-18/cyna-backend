...
class AddressControl: 
    ...
    def add(request, data) -> Optional[Address]:
        try:
            ...
            if data.complement != "" or data.complement is not None:
            ...
            address = AddressService.add(user, data.type, data.street, data.number, data.complement, data.zipCode, data.city, data.region, data.country)
            if address:
    ...
    def get_all_by_user(request) -> Optional[List[Address]]:
        try:
            ...
            return [address.to_json() for address in addresses] if addresses else HttpError(404, "Addresses not found")
        except Exception as e:
    ...
    def update(data) -> Optional[Address]:
        try:
            ...
            address = AddressService.update(data.id, data.type, data.street, data.number, data.complement, data.zipCode, data.city, data.region, data.country)
